...
export const RoundReviewV2Schema = z
  .object({
    ...
    // Filled deterministically by sessions.ts (post-LLM) from interview_turns.answer_text.
    // LLM is instructed to emit "" — the API layer overrides regardless of LLM behavior.
    raw_answer: z.string().default(""),
    // Generated by ReportAgent in the same LLM call (no extra round trip).
    ai_suggested_answer: z.string().default(""),
    // M8.2: first-person readable markdown answer drafted by draftReadableAnswer.
    // Populated by M8.3+ streaming drafter; defaults to "" for backwards compat with old reports.
    ai_suggested_answer_markdown: z.string().default(""),
  })
  .strict();
...
export const ReportAgentOutputSchema = z
  .object({
    ...
    round_reviews_v2: z.array(RoundReviewV2Schema).default([]),
    // next_actions_v2 removed from LLM output: the DarkActionCard UI is gone
    // (per "下一步 also 取消" 改动), and the LLM occasionally emits legacy v3.1
    // style values like "friendly_guided" which Zod rejects → blocks the
    // entire report render. Drop the field from the agent contract.
  })
  .strict();
